...
if (args.length === 0) {
    logger.log(
        ...
        'Please provide a component name. Example: npm run resume:article ArticleTestimonials'
    )
    process.exit(1)
}
...
if (!componentName.startsWith('Article')) {
    logger.log(
        logger.LogTypes.ERROR,
        "Invalid article name. Articles should start with the word 'Article'. Eg: ArticleTestimonials or ArticleTimeline"
    )
    process.exit(1)
}
...
if (!sectionBodyContent.includes(importStatement)) {
    ...
    sectionBodyContent = sectionBodyContent.replace(
        importRegex,
